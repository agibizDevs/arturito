          ...
          if (data) {
            data.servicios.item.forEach(function (flag) {
              if(!flag.respuestaServicio.includes("Fuera")){
                msg.send(`:autobus: Recorrido : ${flag.servicio}, Primer bus : ${flag.horaprediccionbus1} a ${flag.distanciabus1} metros y el segundo : ${flag.horaprediccionbus2} a ${flag.distanciabus2} metros.`);
              }else{
                msg.send(`:autobus: Recorrido : ${flag.servicio} Fuera de horario de operación.`);
              }
            });
          } else {
