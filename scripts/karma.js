...
module.exports = robot => {
        ...
        return robot.adapter.client.web.users.list().then(users => {
          ...
          if (typeof user2 === 'undefined' || user2 === null) {
            ...
            localUsers[user1.id] = {
              ...
              slack: {
                id: user1.id, team_id: user1.team_id, name: user1.name,
                deleted: user1.deleted, status: user1.status, color: user1.color,
                real_name: user1.real_name, tz: user1.tz, tz_label: user1.tz_label,
                tz_offset: user1.tz_offset, profile: user1.profile, is_admin: user1.is_admin,
                is_owner: user1.is_owner, is_primary_owner: user1.is_primary_owner, is_restricted: user1.is_restricted,
                is_ultra_restricted: user1.is_ultra_restricted, is_bot: user1.is_bot, presence: 'active'
              },
              room: 'random', karma: 0
            }
            robot.brain.save()
      ...
      const usersForToken = token => {
        return new Promise((resolve, reject) => {
          let user
          let test = robot.brain.userForName(token)
      ...
      robot.hear(/^karma(?:\s+@?(.*))?$/, response => {
        ...
        if (['todos', 'all'].includes(targetToken.toLowerCase())) {
          ...
          response.send(`Karma de todos: ${JSON.stringify(users)}`)
        } else if (targetToken.toLowerCase().split(' ')[0] === 'reset') {
          const thisUser = response.message.user
          console.log(thisUser.name.toLowerCase());
          if (thisUser.name.toLowerCase() !== 'cvicuna') {
            return response.send('Tienes que ser admin para realizar esta función.')
          }
